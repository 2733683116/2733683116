...
def usr_in():
    ...
    try:
        usr_files = open('usr_info.text', 'r')
        usr_file_ = pickle.load(usr_files)
    except:
        pass
    f = 1
    for i in usr_name:
    ...
    for ii in usr_pwd:
        if 255 >= ord(ii) >= 128:
            k = 0
    if usr_name in usr_file_:
        if usr_pwd == usr_file_[usr_name]:
    ...
    def sign_cg():
        ...
        for ii in np:
            if 255 >= ord(ii) >= 128:
                k = 0
        try:
            usr_files = open('usr_info.text', 'rb')
            exist_usr_info = pickle.load(usr_files)
        ...
        if nn in exist_usr_info:
            ...
            usr_files = open('usr_info.text', 'w')
            usr_file = usr_files.write('nn:np')
            usr_files.close()
